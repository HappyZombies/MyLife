import PlayerActivity from "../data/AbstractPlayerActivity";

class School extends PlayerActivity {
  constructor(){
    super();
  }
  executeActivity(player, event) {
    super.executeActivity(player, event);
    this.player = player;
    this.event = event;
    this._studyLeisureTime = this.player.actions.LeisureTime.STUDY.active
      ? 0.1
      : 0;
  ...
  startSchool() {
    if (this.player.age === 6) {
      this.event.addSmallEvent({
        message: `${this.player.name} started school.`
      });
      this.player.inSchool = true;
    }
    if (this.player.age === 18) {
      this.event.addSmallEvent({
        message: `${this.player.name} graduated from High School.`
      });
  ...
  _inSchoolIntelligence() {
    if (this.player.intelligence >= 85) {
      this.player.intelligence =
        this.player.intelligence +
        this.intelligenceValues.IN_SCHOOL_ABOVE_AVERAGE;
    } else if (
      this.player.intelligence <= 84 &&
      this.player.intelligence >= 25
    ) {
      this.player.intelligence =
        this.player.intelligence + this.intelligenceValues.IN_SCHOOL_AVERAGE;
    } else {
      this.player.intelligence =
        this.player.intelligence +
        this.intelligenceValues.IN_SCHOOL_BELOW_AVERAGE;
    }
  }
  ...
  _notInSchoolIntelligence() {
    if (this.player.intelligence >= 90) {
      this.player.intelligence =
        this.player.intelligence +
        this.intelligenceValues.NOT_IN_SCHOOL_REALLY_SMART;
    } else if (this.player.intelligence <= 25) {
      this.player.intelligence =
        this.player.intelligence + this.intelligenceValues.NOT_IN_SCHOOL;
    }
  }
